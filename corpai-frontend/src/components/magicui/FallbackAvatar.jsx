/**
 * FallbackAvatar — avatar generativo via WebGL.
 * Cores derivadas do hash do nome; anima no hover.
 * Usado quando o usuário não definiu uma foto de perfil.
 *
 * Adaptado de um snippet de referência do 21st.dev (TS→JSX).
 */
...
function renderToCanvas(targetCanvas, uniforms, time) {
  ensureGL();
...
export default function FallbackAvatar({
  ...
  const getUniforms = useCallback(() => {
    if (!uniformsRef.current) uniformsRef.current = computeUniforms(name || '');
